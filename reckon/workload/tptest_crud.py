...
class TPTestCRUD(t.AbstractKeyGenerator):
    def __init__(
        self,
        max_key: int,
        req_per_sec: int,
        ...
        self._op_timers = [
            ...
            # ScaleUp - Read - ScaleDown - Read, 20% Writes
            {
                "op": "u-u",
                "cnt": 1,
                "loop_start": True,
                "loop_cnt": req_per_sec * 6
            },
            {
                "op": "r",
                "cnt": 4
    ...
    def _gen_op_kind(self):
        ...
        if self._op_count < curr_op["cnt"]:
            ...
            if curr_op.get("loop_start", False) and not self._looping:
                self._looping = True
                self._loop_idx = self._op_index
                self._loop_counter = curr_op["loop_cnt"]
            if curr_op.get("loop_end", False) and self._looping:
                self._loop_counter -= 1
                self._op_index = self._loop_idx
