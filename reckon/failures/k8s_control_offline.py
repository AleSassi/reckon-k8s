...
class IntermittentControlOffline(t.AbstractFault):
    def __init__(
        self,
    ...
    def initiate_failure(self):
        target_cp = [h for h in self._cluster if h.node_type is t.KubeNodeType.ControlPlane][2] # cp3 is always distributed to the edge (and very far away)
        major_partition = [h for h in self._cluster if not h == target_cp]
        self._shared.fault_state = threading.Event()

        def thread_fn(self=self, target=target_cp):
...
class IntermittentControlOfflineFailure(t.AbstractFailureGenerator):
